    ...
    <footer id="footer-contact" className="relative isolate overflow-hidden bg-dice-ink pt-20 sm:pt-24">
      ...
      <Container>
        <div className="grid grid-cols-1 gap-12 pb-16 sm:grid-cols-2 sm:gap-10 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr] lg:gap-12">
            ...
            <div className="flex items-center gap-3">
              {socials.map((s) => (
                <a
                  ...
                  href="#"
                  aria-label={s.label}
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-white/70 transition-all duration-300 hover:-translate-y-0.5 hover:border-dice-cyan/50 hover:text-dice-cyan"
                >
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              {footerLinks.quickLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="text-sm text-white/60 transition-colors hover:text-dice-cyan">
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          {/* Column 3 — Support */}
          <div className="flex flex-col gap-4">
            <h3 className="font-display text-sm font-semibold uppercase tracking-[0.16em] text-white">Support</h3>
            <ul className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3.5 text-sm text-white/60">
              <li className="flex items-start gap-3">
                <Icon name="headset" className="mt-0.5 h-4 w-4 shrink-0 text-dice-cyan" />
